...
const mentionRegex = new RegExp(
  mentionRegexPattern + mentionSymbolPattern + mentionUsernamePattern,
  "g"
);
...
function MentionsTypeaheadMenuItem({
  ...
  return (
    <li
      ...
      onClick={(e) => {
        console.log("test");
        e.stopPropagation(); // Prevents unnecessary bubbling but ensures li click is handled
        onClick(); // Handle the main click action here
      }}
    >
      <div
        tabIndex={-1}
        className="overflow-x-hidden overflow-y-hidden flex items-center relative px-0 py-0 mx-0 my-0 w-full border-b border-border"
      >
        <img
          src={option.image}
          alt={option.name}
          className="absolute top-0 left-0 opacity-0 z-50 w-[280px] h-[56px]"
          width={280}
          height={56}
        />
        <div className="flex flex-row flex-shrink px-1 py-0.5">
          ...
          <div
            ...
            className="justify-center flex flex-col relative w-full"
          >
            <span className="font-medium text-[15px] w-full">
              {option.username}
            </span>
            <span className="whitespace-nowrap max-w-full font-light text-[15px] opacity-45">
              {option.name}
            </span>
          </div>
        </div>
...
type Props = {
  isRequestingFromStories?: boolean;
};

export function MentionsPlugin({
  isRequestingFromStories,
  ...
  const options = useMemo(() => {
    if (results !== null) {
      return results
        .map((result: { name: any; image: any; username: any; id: any }) => {
    ...
    (
      ...
      realeditor.update(() => {
        const mentionNode = $createMentionNode(
          ...
          selectedOption.id,
          isRequestingFromStories as boolean
        );
        if (nodeToReplace) {
          ...
          ? ReactDOM.createPortal(
              <>
                {loading ? (
                  ...
                  >
                    <div
                      className="overflow-y-auto max-h-52 rounded-xl bg-[rgba(16,16,16,1)]"
                      id="main-pop"
                    >
                  ...
                  >
                    <div
                      className=" rounded-[16px] w-full overflow-x-hidden drop-shadow"
                      id="main-pop"
                    >
                            ...
                            <MentionsTypeaheadMenuItem
                              ...
                              onClick={() => {
                                ...
                                try {
                                  ...
                                  console.error(
                                    "Error in selectOptionAndCleanUp:",
                                    error
                                  );
                                }
                              }}
