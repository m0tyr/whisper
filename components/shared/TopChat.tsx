...
const TopChat = ({ user, _id }: any) => {
    const [isOpen, setIsOpen] = useState(false); // State to track whether the component should be open

    const handleInputClick = () => {
        setIsOpen(true); // Set isOpen to true when the input is clicked
    };

    const togglePopup = () => {
        setIsOpen(false); // Set isOpen to false when the background is clicked
    };
