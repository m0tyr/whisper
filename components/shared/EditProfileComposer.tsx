...
interface EditProfileComposerProps {
...
const EditProfileComposer = ({
    ...
    if (parentDiv) {
      ...
      if (childCount >= 9) {
        if (event.key === "Enter" || event.keyCode === 13) {
          ...
          event.currentTarget.textContent?.length! >= composerFieldInputLimit
        ) {
          event.preventDefault();
        }
      } else if (
  ...
  const handlePaste = (event: React.ClipboardEvent<HTMLDivElement>) => {
    ...
    if (
      pastedText.length + (event.currentTarget.textContent?.length || 0) >
      composerFieldInputLimit
    ) {
      event.preventDefault();
    }
  ...
  const handleContentChange = () => {
    ...
    if (textContent.trim() === "") {
      ...
      onUpdateData(
        textContent,
        composerEditProfileFieldType.toString().toLowerCase()
      );
      triggerCloseDialog();
    }
  };
  ...
  return (
    <>
      <div
        className="fixed left-1/2 top-1/2  transform -translate-x-1/2 -translate-y-[70%] "
        id="editableDiv"
      >
        <div className="flex flex-col">
          <div className=" flex-row h-10 grid grid-cols-[1fr_max-content_1fr] px-4">
            <p className="inline font-light cursor-pointer">
              <button onClick={UndoContentChange}>Annuler</button>
            </p>
            <p className="inline font-semibold ">
              Modifier votre {composerEditProfileFieldType}
            </p>
            <p className="inline ml-auto font-light text-sky-500 cursor-pointer">
              {" "}
              <button onClick={handleContentChange}>Terminé</button>
            </p>
          </div>
          <div
...
export default EditProfileComposer;
