...
const RenderMainViewWhisperPost = ({ post }: Props) => {
  const isInViewingView = true;
  const ViewportIndicator: WhisperViewportTypes = "direct";
  return (
    <WhisperPost
      key={`${post._id}`}
      post={{
        ...
        comments: [
          {
            ...
            childrens: post.children.map(
              (child: {
                author: { image: any; username: any; id: any };
                content: any;
                createdAt: any;
              }) => ({
                author: {
        ...
        isInViewingView: isInViewingView,
