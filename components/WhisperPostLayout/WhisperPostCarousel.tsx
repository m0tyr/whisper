...
const WhisperPostCarousel = ({ DataArray, widthprovider, srcprovider, typeprovider, arprovider, setShowImage, showImage }: Props) => {
    ...
    return (
        <AnimatePresence>
            <motion.div ref={carouselRef} className={`overflow-hidden ${ViewportIndicator === "default" && isOnlyMediaPost ? 'mt-[18px]' : 'mt-3'}  active:cursor-grabbing cursor-grab`} whileTap={"grabbing"}>
                <motion.div
                    key={id}
                    ref={fullcarouselRef}
